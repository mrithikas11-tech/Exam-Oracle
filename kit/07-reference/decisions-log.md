...
| 16:30 | **Bring your own LLM:** `exam-oracle llm --provider claude|chatgpt|gemini|ollama|custom` writes Cognee 1.5.4's own settings (LLM_PROVIDER/LLM_MODEL/LLM_ENDPOINT/LLM_API_KEY, EMBEDDING_*) into the git-ignored `.env`; key via hidden prompt. Claude default model `claude-opus-5`; non-OpenAI presets use local fastembed embeddings (all-MiniLM-L6-v2, 384 dims). Dependency now `cognee[anthropic,fastembed]==1.5.4` (Cognee's own extras) | Product owner: each team member uses the LLM they prefer; Claude has no embedding API | A |
