...
class SkipList:
    ...
    @classmethod
    def load(cls, path: str | None, course: str | None = None) -> "SkipList":
        """The given (or builtin) list, plus data/courses/<course>/skip_list.txt when that file exists."""
        if path in (None, "", "builtin"):
            ...
            source = str(path)
        if course:
            from oracle import config as structure_config  # role B's config: REPO_ROOT / data

            course_list = structure_config.DATA_DIR / "courses" / check_course(course) / "skip_list.txt"
            if course_list.is_file():
                text += "\n" + course_list.read_text()
                source += f"+data/courses/{course}/skip_list.txt"
        urls, hashes = set(), set()
